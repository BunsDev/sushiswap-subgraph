...
import { getReaper, getServer } from '../entities'
import { Serving } from '../../generated/schema'
import { Factory as FactoryContract } from '../../generated/SoulReaper/Factory'
import { LogConvert } from '../../generated/SoulReaper/SoulReaper'


export function handleLogConvert(event: LogConvert): void {
  log.info('[SoulReaper] Log Convert {} {} {} {} {} {}', [
    ...
    event.params.amountSOUL.toString()
  ])

  const maker = getReaper(event.block)
  ...
  serving.amountSoul = event.params.amountSOUL
  serving.block = event.block.number
  serving.timestamp = event.block.timestamp
  serving.save()

  maker.soulServed = maker.soulServed.plus(event.params.amountSOUL)
  maker.totalServings = maker.totalServings.plus(BIG_DECIMAL_ONE)
  maker.save()

  server.soulServed = server.soulServed.plus(event.params.amountSOUL)
