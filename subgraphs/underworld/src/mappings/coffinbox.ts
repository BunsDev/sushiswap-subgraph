...
import { UNDERWORLD_MEDIUM_RISK_MASTER_CONTRACT_ADDRESSES } from '../constants'
...
export function onLogDeploy(event: LogDeploy): void {
  ...
  if (UNDERWORLD_MEDIUM_RISK_MASTER_CONTRACT_ADDRESSES.includes(event.params.masterContract.toHex())) {
    createUnderworldPair(event)
  }
}