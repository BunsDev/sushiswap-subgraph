...
import { UnderworldPair, UnderworldPairDaySnapshot, UnderworldPairHourSnapshot } from "../../generated/schema"
import { DAY_IN_SECONDS, HOUR_IN_SECONDS } from "../constants"


export function updateUnderworldPairSnapshots(timestamp: BigInt, pair: UnderworldPair): void {
    updatePairHourSnapshot(timestamp, pair)
    updatePairDaySnapshot(timestamp, pair)
  }


function updatePairHourSnapshot(timestamp: BigInt, pair: UnderworldPair): void {
    let id = getPairHourSnapshotId(pair.id, timestamp)
  
    let snapshot = UnderworldPairHourSnapshot.load(id)
  
    if (snapshot === null) {
      snapshot = new UnderworldPairHourSnapshot(id)
  ...
  function updatePairDaySnapshot(timestamp: BigInt, pair: UnderworldPair): void {
    let id = getPairDaySnapshotId(pair.id, timestamp)
  
    let snapshot = UnderworldPairDaySnapshot.load(id)
  
    if (snapshot === null) {
      snapshot = new UnderworldPairDaySnapshot(id)
